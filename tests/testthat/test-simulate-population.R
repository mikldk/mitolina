...
test_that("male_individuals_generations: get_haplotypes_individuals / get_haplotypes_pids haps", {
  expect_equal(length(haps_3gen), length(pids_3gen))
  expect_equal(length(haps_3gen), length(haps_pids_3gen))
...
test_that("ped: get_individual / get_haplotypes_individuals / get_haplotypes_in_pedigree haps", {
  expect_equal(length(haps_indv_ped), length(haps_ped))
  expect_equal(length(haps_indv_ped), length(pids_ped))
  expect_equal(haps_indv_ped, haps_ped)
})
